...
const createtitle = async (input_information) => {
    ...
    let api_url = "https://app.rakuten.co.jp/services/api/BooksBook/Search/20170404?format=json&isbn=&applicationId=ここにAPIID";
    //urlコンストラクターを定義
    ...
    fetch(serach_url)
        ...
        .then(json => {
            ...
            element.insertAdjacentHTML('beforeend',`<img src=${g_photo} alt="書籍の表紙" title=${g_title}></img>`);

            element.insertAdjacentHTML('beforeend',`<p>${g_title}</p>`);
